...
tf.app.flags.DEFINE_string('image_dir', 'images/multi_label',
...
tf.app.flags.DEFINE_integer('eval_step_interval', 200,
...
tf.app.flags.DEFINE_string('bottleneck_dir', 'tmp/bottleneck',
...
# TODO: We have to apply data augmentation, just not every time, this should be done once and this code should be removed from this script. Low priority
        ...
        for category in ['training', 'testing', 'validation']:
            ...
            for index, unused_base_name in enumerate(category_list):
                ...
                if how_many_bottlenecks % 100 == 0:
                    print(str(how_many_bottlenecks) + ' bottleneck files loaded.')


def get_ground_truth(labels_file, labels, class_count):
    '''
    This method turns labels .txt files description to a numpy array of size (class_count,), with binary values, i.e.
    if labels correspondent to current image the value of this label position in the array is 1.0 otherwise is 0.0
    :param labels_file: File containing the labels for the training image
    :param labels: File containing all possible labels.
...
def add_final_training_ops(class_count, final_tensor_name, bottleneck_tensor):
    ...
    with tf.name_scope('train'):
        # Set up optimization algorithm, and loss target
        # T-O-D-O: change to Adam optimization. ---- DONE
...
def main(_):
    ...
    # Get the number of classes of the data set, in our case 9.
    class_count = len(labels)
    ...
    if class_count == 0:
        ...
        return -1
    if class_count == 1:
        print('Only one valid label found inside {} - multiple classes are needed for classification.'.format(
            ALL_LABELS_FILE))
        return -1

    # See if the command-line flags mean we're applying any distortions.
    # TODO: Shall we apply distortions only once or in every training?
    ...
    best_lr = -1
    best_acc = -1
    # Begin experiments with the already cached bottlenecks.
    learning_rates = np.linspace(0.0005, 0.000001, 20)  # Learning rates to iterate.
    for lr in learning_rates:
        # Change current Flags for experiments.
        FLAGS.learning_rate = lr
        print('INFO - Current learning rate: ', FLAGS.learning_rate)

        # Merge all the summaries and write them out to tmp/retrain_logs
        merged = tf.summary.merge_all()

        # TODO: save summaries final directory depending on the current experiment.
        train_path = FLAGS.summaries_dir + '/train' + '/leaning_rate-' + str(lr)
        val_path = FLAGS.summaries_dir + '/validation' + '/leaning_rate-' + str(lr)
        if tf.gfile.Exists(train_path):
            tf.gfile.DeleteRecursively(train_path)
        if tf.gfile.Exists(val_path):
            tf.gfile.DeleteRecursively(val_path)
        tf.gfile.MakeDirs(train_path)
        tf.gfile.MakeDirs(val_path)
        train_writer = tf.summary.FileWriter(train_path, sess.graph)
        validation_writer = tf.summary.FileWriter(val_path)
        print('INFO - Summaries being saved at : ' + train_path)
        ...
        # TODO: Change this code to access images in 'images/test_photos' folder, and use them as test data set
        test_bottlenecks, test_ground_truth = get_random_cached_bottlenecks(sess, image_lists, FLAGS.test_batch_size,
                                                                            'testing',
        ...
        if (test_accuracy * 100 > best_acc):
            best_acc = test_accuracy * 100
            best_lr = lr
    # Save pseudo log of best experiment.
    best_hyper_params = open('best_hyper_params.txt','w')
    best_hyper_params.truncate()
    best_hyper_params.write(str(best_acc) + '\n' + str(best_lr))
    best_hyper_params.close()
    # Write out the trained graph and labels with the weights stored as constants.
    # TODO: Only save as constants for final model
